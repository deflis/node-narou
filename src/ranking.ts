...
export default class RankingBuilder {
  ...
  >(
    fieldsOrOptions?: TFields | TFields[] | ExecuteOptions,
    optOrOptions?: TOpt | ExecuteOptions,
    options?: ExecuteOptions
  ): Promise<RankingResult<SearchResultFields<TFields>>[]> {
    const { fields, opt, executeOptions } = this.resolveExecuteArgs(
      fieldsOrOptions,
      optOrOptions,
      options
    );

    const ranking = await this.execute(executeOptions);
    ...
    const result = await builder.execute(executeOptions);
  ...
  private resolveExecuteArgs<
    TFields extends Fields,
    TOpt extends OptionalFields | undefined
  >(
    fieldsOrOptions?: TFields | TFields[] | ExecuteOptions,
    optOrOptions?: TOpt | ExecuteOptions,
    options?: ExecuteOptions
    ...
    let fields: TFields | TFields[] = [] as TFields[];
    let opt: TOpt | undefined;
    let executeOptions: ExecuteOptions | undefined;

    if (Array.isArray(fieldsOrOptions) || typeof fieldsOrOptions === "string") {
      fields = fieldsOrOptions;
    } else if (fieldsOrOptions) {
      executeOptions = fieldsOrOptions;
    }

    if (Array.isArray(optOrOptions) || typeof optOrOptions === "string") {
      opt = optOrOptions as TOpt;
    } else if (optOrOptions) {
      executeOptions = optOrOptions;
    }
    ...
    return { fields, opt, executeOptions };
  }
}
